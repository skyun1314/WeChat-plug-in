...
public class DexFileWriter extends DexFileVisitor {
    ...
    void buildMapListItem(boolean doResort) {
        ...
        List<StringIdItem> stringIdList = Arrays.asList(cp.strings.values().toArray(new StringIdItem[]{}));
        if(doResort)	Collections.sort(stringIdList);
        SectionItem<StringIdItem> stringIdSection = new SectionItem<>(
                SectionType.TYPE_STRING_ID_ITEM, stringIdList);
        
        List<TypeIdItem> typeIdList = Arrays.asList(cp.types.values().toArray(new TypeIdItem[]{}));
        if(doResort)	Collections.sort(typeIdList);
        SectionItem<TypeIdItem> typeIdSection = new SectionItem<>(
                SectionType.TYPE_TYPE_ID_ITEM, typeIdList);
        
        List<ProtoIdItem> protoIdList = Arrays.asList(cp.protos.values().toArray(new ProtoIdItem[]{}));
        if(doResort)	Collections.sort(protoIdList);
        SectionItem<ProtoIdItem> protoIdSection = new SectionItem<>(
                SectionType.TYPE_PROTO_ID_ITEM, protoIdList);
        
        List<FieldIdItem> fieldIdList = Arrays.asList(cp.fields.values().toArray(new FieldIdItem[]{}));
        if(doResort)	Collections.sort(fieldIdList);
        SectionItem<FieldIdItem> fieldIdSection = new SectionItem<>(
                SectionType.TYPE_FIELD_ID_ITEM, fieldIdList);
        
        List<MethodIdItem> methodIdList = Arrays.asList(cp.methods.values().toArray(new MethodIdItem[]{}));
        if(doResort)	Collections.sort(methodIdList);
        SectionItem<MethodIdItem> methodIdSection = new SectionItem<>(
                SectionType.TYPE_METHOD_ID_ITEM, methodIdList);
        
        List<ClassDefItem> classDefItemList = cp.buildSortedClassDefItems();
        SectionItem<ClassDefItem> classDefSection = new SectionItem<>(
                SectionType.TYPE_CLASS_DEF_ITEM, classDefItemList);
        ...
        if(doResort) {
	        ...
	        {
	        	classDefItemList.get(i).resortFieldAndMethod();
	        }
        }
        ...
        if(doResort)	Collections.sort(cp.stringDatas);
    ...
    public byte[] toByteArray(boolean doResort) {

        // init structure for writing
        buildMapListItem(doResort);
