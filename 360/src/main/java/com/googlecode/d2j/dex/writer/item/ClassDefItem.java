...
public class ClassDefItem extends BaseItem {
    ...
    public void putClazzData(ClassDataItem classData) {
    	this.classData = classData;
    	Checker.appendNamesInParcelableSet(this);
    	Checker.appendCustomViewSet(this);
    }
    
    public boolean isParcelable() {
    	if(interfaces == null || interfaces.items == null || interfaces.items.isEmpty()) {
    		return false;
    	}
    	for(int i = 0; i < interfaces.items.size(); ++i)
    	{
    		TypeIdItem typ = interfaces.items.get(i);
    		String typName = typ.descriptor.stringData.string;
    		if(typName.contains("Parcelable") || typName.contains("Serializable") || typName.contains("Externalizable"))	return true;
    	}
    	return false;
    }
    ...
    public boolean isViewChild() {
    	...
    	String clzName = superclazz.descriptor.stringData.string;
    	clzName = clzName.substring(clzName.lastIndexOf("/") + 1, clzName.indexOf(";"));
    	if(clzName.endsWith("View") || clzName.startsWith("View"))	return true;
    	return false;
    }
    
    public void resortFieldAndMethod() {
    	if (classData == null) {
            return;
        }
    	
    	classData.resortFieldAndMethod();
    	...
    	if(staticValues != null) {
            ...
            if(count >= 0) {
            	staticValues.value.values.clear();
            	for (int i = 0; i <= count; i++) {
                    EncodedField f = fs.get(i);
                    EncodedValue ev = f.staticValue;
                    if (ev == null) {
                    	staticValues.value.values.add(EncodedValue.defaultValueForType(f.field.getTypeString()));
                    } else {
                    	staticValues.value.values.add(ev);
                    }
                }
            }
    	}
