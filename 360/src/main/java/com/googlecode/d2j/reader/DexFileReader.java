...
public class DexFileReader implements BaseDexFileReader {
  ...
  static int sint(byte[] data, int offset) {
    return (data[offset + 3] << 24) | ((0xFF & data[offset + 2]) << 16) | ((0xFF & data[offset + 1])
        << 8) | ((0xFF & data[offset]));
  }
   ...
   */
  @Override public void accept(DexFileVisitor dv) {
    this.accept(dv, 0);
  }

  @Override public List<String> getClassNames() {
   ...
   */
  @Override public void accept(DexFileVisitor dv, int config) {
    for (int cid = 0; cid < class_defs_size; cid++) {
   ...
   * {@link #SKIP_FIELD_CONSTANT}
   */
  @Override public void accept(DexFileVisitor dv, int classIdx, int config) {
    ...
    try {
      ...
      if (dcv != null) {
        acceptClass(dcv, source_file_idx, annotations_off, class_data_off, static_values_off,
            config);
        dcv.visitEnd();
      }
    } catch (Exception ex) {
      DexException dexException =
          new DexException(ex, "Error process class: [%d]%s", class_idx, className);
  ...
  private void acceptClass(DexClassVisitor dcv, int source_file_idx, int annotations_off,
      int class_data_off, int static_values_off, int config) {
    ...
    if (class_data_off != 0) {
      ...
      int virtual_methods = (int) readULeb128i(in);
      {
        ...
        for (int i = 0; i < direct_methods; i++) {
          lastIndex =
              acceptMethod(in, lastIndex, dcv, methodAnnotationPositions, paramAnnotationPositions,
                  config, firstMethod);
          firstMethod = false;
        }
        lastIndex = 0;
  ...
  private int acceptMethod(ByteBuffer in, int lastIndex, DexClassVisitor cv,
      Map<Integer, Integer> methodAnnos, Map<Integer, Integer> parameterAnnos, int config,
      boolean firstMethod) {
    int offset = in.position();
    int diff = (int) readULeb128i(in);
    int method_access_flags_pos = in.position();
    int method_access_flags = (int) readULeb128i(in);
    int code_off_pos = in.position();
    ...
    String[] shorty = new String[method.getParameterTypes().length + 1];
    for (int i = 1; i <= method.getParameterTypes().length; ++i) {
      shorty[i] = method.getParameterTypes()[i - 1].substring(0, 1);
    }
    shorty[0] = method.getReturnType().substring(0, 1);
    //String mShorty = "".join("", shorty);
    String desc = method.getDesc();
    String funcInfo =
        method.getOwner() + method.getName() + desc.substring(0, desc.indexOf(')') + 1);
    /**非dex2jar自身代码**/
    Encrypter.methodAccessFlagsPos.put(method.getOwner() + "->" + method.getName(),
        method_access_flags_pos);
    Encrypter.func2codePos.put(method.getOwner() + "->" + method.getName(),
        code_off_pos);//当前相对文件的游标
    Encrypter.func2codeOff.put(method.getOwner() + "->" + method.getName(), code_off);//当前游标读取解析的值
    /**非dex2jar自身代码**/
    if (method.getName().equals("onCreate") && method_access_flags == 260) {
      System.out.println(method.getOwner() + "&&*******");
      System.out.println(code_off + "&&*******");
      //System.out.println(method.getName() + "&&*******");
      System.out.print("funcInfo  " + funcInfo + " 结束");
    }
    if (Encrypter.func2code.containsKey(funcInfo)) {
      //System.out.println(funcInfo+"%%%%%%");
      //System.out.println(mShorty+"%%%%%%");
      Number num = (Number) (Encrypter.func2code.get(funcInfo));
      code_off = num.intValue();
      method_access_flags = 4;
    }
    ...
    if (0 == (method_access_flags & DexConstants.ACC_CONSTRUCTOR) && (method.getName()
        .equals("<init>") || method.getName().equals("<clinit>"))) {
      ...
      if (dmv != null) {
        ...
        if (code_off != 0) {
          ...
          if (keep) {
            ...
            if (dcv != null) {
              try {
                acceptCode(code_off, dcv, config,
                    (method_access_flags & DexConstants.ACC_STATIC) != 0, method);
              } catch (Exception e) {
                throw new DexException(e, "while accept code in method:[%s] @%08x",
                    method.toString(), code_off);
              }
            }
          }
        }
  ...
  private void findLabels(byte[] insns, BitSet nextBit, BitSet badOps,
      Map<Integer, DexLabel> labelsMap, Set<Integer> handlers, Method method) {
    ...
    while (!q.isEmpty()) {
      ...
      try {
        ...
        badOps.set(offset);
        WARN("GLITCH: %04x %s | not enough space for reading instruction", offset,
            method.toString());
  ...
  private void travelInsn(Map<Integer, DexLabel> labelsMap, Queue<Integer> q, byte[] insns,
      int offset) {
  ...
  private void findTryCatch(ByteBuffer in, DexCodeVisitor dcv, int tries_size, int insn_size,
    ...
    for (int i = 0; i < tries_size; i++) { // try_item
      ...
      handlerIn.position(
          encoded_catch_handler_list + handler_offset);// move to encoded_catch_handler
  ...
  /* package */void acceptCode(int code_off, DexCodeVisitor dcv, int config, boolean isStatic,
      Method method) {
  ...
  private void acceptInsn(byte[] insns, DexCodeVisitor dcv, BitSet nextInsn, BitSet badOps,
    ...
    Op[] values = Op.ops;
    for (int offset = nextInsn.nextSetBit(0); offset >= 0;
